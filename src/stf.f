...
c   vsn 1.7:  B90210 added "-L" option
c
...
c
      ...
      If (Line(1:1) .eq. '|') then
        GotPipe = .True.
        if (.not.GotFlds) call GetFlds(Line,FldNam,IFa,IFb,NF)
        GotFlds = .true.
      end if
...
200   If (.not.List) Go to 300
      ...
      Do 220 I = 1, NFlds
        ...
        if (ShoWidths) then
          write (NumStr,'(i4,'':'',i6,''-'')') I, IFa(I)
          write (Fmt,'(i5)') IFb(I)
          NumStr(13:17) = AdjustL(Fmt)
          write(NumStr(18:22),'(i5)') IFb(I)-IFa(I)+1
          write(6,'(a)') NumStr//Field(1:lnblnk(Field))
        else                           ! this uses older Fortran
...
c-----------------------------------------------------------------------
c%%%WinMac: b 26
6000  Format('STF 1.7  B90210  (Subset Table Files)'/
     + /'usage:  stf <filename> [options]'//
     + 'where [options] may be:'/
     + '       -l    (list field names by number)'/
     + '       -L    (list field names and widths)'/
...
6001  Format('\  Subset generated by STF 1.7 B90219 from file '$)
6002  Format(I2$)
6003  Format(I3$)
6004  Format(I4$)
6005  Format(I5$)
